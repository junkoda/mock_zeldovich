...
{
  PowerSpectrum* const pdd=
    (PowerSpectrum*) malloc(sizeof(PowerSpectrum)*3); assert(pdd);
  PowerSpectrum* const pdt= pdd+1;
  PowerSpectrum* const ptt= pdt+1;
  ...
  int nalloc= 1000;
  double* buf= (double*) malloc(sizeof(double)*4*nalloc);

  char line[128];
  int nlines= 0;
  double k, Pdd, Pdt, Ptt;
  ...
  while(fgets(line, 127, fp)) {
    if(nlines == nalloc) {
      fprintf(stderr, "Reallocating power spectrum table %d -> %d\n",
		 nalloc, 2*nalloc);
      nalloc *= 2;
      buf= (double*) realloc(buf, sizeof(double)*4*nalloc); assert(buf);
    }
    
    if(line[0] == '#')
      continue;    

    int ret= sscanf(line, "%lg %lg %lg %lg", &k, &Pdd, &Pdt, &Ptt);
    if(ret != 4) {
      fprintf(stderr, "Error: 4 columns required for k, Pdd, Pdt, and Ptt\n");
  ...
  fprintf(stderr, "Read %d pairs of k Pdd Pdt Ptt from %s\n", nlines, filename);

  // Allocate ps->log_k, ps->log_P and fill the arrays
  //double* const v_logk=
  pdd->k   = (double*) malloc(4*nlines*sizeof(double)); assert(pdd->k);
  pdd->P    = pdd->k + nlines;
  //pdd->log_k= ps->P + nlines;
  //pdd->log_P= ps->log_k + nlines;
  ...
  for(int j=0; j<nlines; j++) {
    pdd->k[j] = buf[4*j];
    pdd->P[j] = buf[4*j + 1];
    //pdd->log_k[j]= log(buf[2*j]);
    //pdd->log_P[j]= log(buf[2*j + 1]);
    pdt->P[j] = buf[4*j + 2];
    ptt->P[j] = buf[4*j + 3];
  }
  free(buf);
  
  pdd->n= pdt->n= ptt->n= nlines;
